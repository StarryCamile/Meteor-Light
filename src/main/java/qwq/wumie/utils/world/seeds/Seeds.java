...
import meteordevelopment.meteorclient.events.world.SeedChangedEvent;
...
public class Seeds extends System<Seeds> {
    ...
    public void setSeeds(HashMap<String, Seed> seeds) {
        this.seeds = seeds;
    }

    public HashMap<String,Seed> getSeeds() {
        return seeds;
    }

    public Seed getSeedForIP(String ip) {
        for (Map.Entry<String, Seed> seed : seeds.entrySet()) {
            if (seed.getValue().ip.equalsIgnoreCase(ip)) {
                return new Seed(seed.getValue().seed,seed.getValue().version, seed.getValue().ip);
            }
        }
        return null;
    }
    ...
    public Seed getSeed() {
        if (mc.isIntegratedServerRunning() && mc.getServer() != null) {
            ...
            return new Seed(mc.getServer().getOverworld().getSeed(), version,mc.getServer().getServerIp());
        }
    ...
    public void setSeed(String seed, MCVersion version) {
        ...
        seeds.put(Utils.getWorldName(), new Seed(numSeed, version,mc.getCurrentServerEntry().address));
        SeedConfigs.save();
