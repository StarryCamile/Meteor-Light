...
package qwq.wumie.utils.misc;

import com.google.common.collect.Multimap;
import com.google.common.collect.MultimapBuilder;
import meteordevelopment.meteorclient.MeteorClient;
import qwq.wumie.systems.modules.misc.NotebotI;
import qwq.wumie.systems.modules.misc.NotebotI.*;
import static meteordevelopment.meteorclient.utils.player.ChatUtils.*;
...
public class NotebotUtils {
    ...
    public static NotebotI.Song parse(Path path) {
    ...
    public static NotebotI.Song parseNl(Path path) {
        Multimap<Integer, NotebotI.Note> notes = MultimapBuilder.linkedHashKeys().arrayListValues().build();
            ...
            for (String s: Files.readAllLines(path)) {
                if (s.startsWith("// Name: ")) {
                    ...
                    try {
                        ...
                        warning("Error trying to parse note: \u00a7o" + s);
                    }
                }
            }
        } catch (IOException e) {
            error("Error reading NL file!");
            e.printStackTrace();
        }
    ...
    public static NotebotI.Song parseMidi(Path path) {
        ...
        try {
            ...
            info(midiFormat.properties().toString());
        ...
        try (InputStream input = Files.newInputStream(path)) {
            ...
            while ((jump = readShort(input)) != 0) {
                ...
                while (readShort(input) != 0) {
                    ...
                    if (key < 0) {
                        MeteorClient.LOG.info("Note @" + tick + " Key: " + key + " is below the 2-octave range!");
                        key = Math.floorMod(key, 12);
                    } else if (key > 25) {
                        MeteorClient.LOG.info("Note @" + tick + " Key: " + key + " is above the 2-octave range!");
            ...
            MeteorClient.LOG.error("Error reading Nbs file!");
            e.printStackTrace();
        }
