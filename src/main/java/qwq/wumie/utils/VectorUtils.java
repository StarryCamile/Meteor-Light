...
public class VectorUtils {
    ...
    public static boolean place(BlockPos pos, FindItemResultPlus item, boolean rotate, int rotationPriority, boolean swingHand, boolean checkEntities, boolean swapBack) {
        if (item.isOffhand()) {
    ...
    public static Direction getPlaceSide(BlockPos pos) {
        for (Direction side : Direction.values()) {
            ...
            if (!state.isReplaceable() && state.getFluidState().isEmpty() && !VectorUtils.isClickable(mc.world.getBlockState(pos.offset(direction)).getBlock())) {
                return direction;
            }
        }
    ...
    public static FindItemResultPlus findInHotbar(Predicate<ItemStack> isGood) {
        if (isGood.test(mc.player.getOffHandStack())) {
            return new FindItemResultPlus(SlotUtils.OFFHAND);
        }

        if (isGood.test(mc.player.getMainHandStack())) {
            return new FindItemResultPlus(mc.player.getInventory().selectedSlot);
        }

        return find(isGood, 0, 8);
    }

    public static FindItemResultPlus find(Item... items) {
    ...
    public static FindItemResultPlus find(Predicate<ItemStack> isGood) {
        return find(isGood, 0, mc.player.getInventory().size());
    }

    public static FindItemResultPlus find(Predicate<ItemStack> isGood, int start, int end) {
        int slot = -1;
        ...
        return new FindItemResultPlus(slot);
    }
