...
import qwq.wumie.systems.music.player.decoder.flac.FlacDecoder;
import qwq.wumie.systems.music.player.decoder.mp3.Mp3Decoder;
import qwq.wumie.systems.music.player.decoder.ogg.OggDecoder;
...
public class APlayer extends InputStream {
    ...
    public APlayer() {
        try {
            new Thread(this::run, "music_run").start();
        ...
        while (true) {
            try {
                ...
                try {
                    ...
                    e.printStackTrace();
                    MusicManager.INSTANCE.warning("获取音乐失败");
                    continue;
                }
                ...
                if (!decoder.set()) {
                    ...
                    if (!decoder.set()) {
                        ...
                        if (!decoder.set()) {
                            MusicManager.INSTANCE.warning("不支持这样的文件播放");
                            continue;
                        }
                    }
                }
                ...
                while (!isClose && AL10.alGetSourcei(index,
                    ...
                    AL10.alSourcef(index, AL10.AL_GAIN, MusicManager.getVolume());
                    Thread.sleep(100);
                }
                if (!reload) {
                    wait = true;
