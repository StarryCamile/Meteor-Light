...
import qwq.wumie.systems.commands.commands.MusicCommand;
import qwq.wumie.systems.hud.elements.MusicHud;
import qwq.wumie.systems.music.MusicManager;
import qwq.wumie.systems.music.objs.HttpResObj;
import qwq.wumie.systems.music.objs.SearchMusicObj;
import qwq.wumie.systems.music.objs.api.music.info.InfoObj;
import qwq.wumie.systems.music.objs.api.music.list.DataObj;
import qwq.wumie.systems.music.objs.api.music.lyric.WLyricObj;
import qwq.wumie.systems.music.objs.api.music.search.SearchDataObj;
import qwq.wumie.systems.music.objs.api.music.search.songs;
import qwq.wumie.systems.music.objs.api.music.trialinfo.TrialInfoObj;
import qwq.wumie.systems.music.objs.api.program.info.PrInfoObj;
import qwq.wumie.systems.music.objs.music.SearchPageObj;
...
public class APIMain {
    ...
    public void login(String code) {
        ...
        if (MusicManager.cookie.cookieStore.containsKey("music.163.com")) {
            List<Cookie> cookies = MusicManager.cookie.cookieStore.get("music.163.com");
            ...
            MusicManager.cookie.cookieStore.put("music.163.com", cookies1);
            MusicManager.saveCookie();
        }
        params.addProperty("countrycode", "86");
        params.addProperty("phone", MusicCommand.phone);
    ...
    private SongInfoObj getMusicDetail(String id, boolean isList) {
        ...
        if (res != null && res.ok) {
            InfoObj temp = MusicManager.gson.fromJson(res.data, InfoObj.class);
            if (temp.isOk()) {
                ...
                TrialInfoObj obj = MusicManager.gson.fromJson(res.data, TrialInfoObj.class);
    ...
    public SongInfoObj getMusic(String id, boolean isList) {
        ...
        if (res != null && res.ok) {
            PrInfoObj temp = MusicManager.gson.fromJson(res.data, PrInfoObj.class);
            if (temp.isOk()) {
                return new SongInfoObj(temp.getAuthor(), temp.getName(),
    ...
    public void setList(String id) {
        final Thread thread = new Thread(() -> {
            ...
            if (res != null && res.ok)
                try {
                    isUpdate = true;
                    DataObj obj = MusicManager.gson.fromJson(res.data, DataObj.class);
    ...
    public LyricSave getLyric(String id) {
        ...
        if (res != null && res.ok) {
            try {
                WLyricObj obj = MusicManager.gson.fromJson(res.data, WLyricObj.class);
    ...
    public SearchPageObj search(String[] name, boolean isDefault) {
        ...
        if (res != null && res.ok) {
            SearchDataObj obj = MusicManager.gson.fromJson(res.data, SearchDataObj.class);
    ...
    public String getListMusic() {
        if (PlayMusic.error >= 10)
            return null;
        if (!isUpdate && PlayMusic.getList().size() != 0) {
            String ID;
            if (MusicHud.INSTANCE.randomPlay.get()) {
                if (PlayMusic.playList.size() == 0)
                    return null;
                else if (PlayMusic.playList.size() == 1)
                    return PlayMusic.playList.get(0).getID();
                ID = PlayMusic.playList.get(new Random().nextInt(PlayMusic.playList.size())).getID();
            } else {
                ID = PlayMusic.playList.get(PlayNow).getID();
                if (PlayNow == PlayMusic.playList.size() - 1)
                    PlayNow = 0;
                else
                    PlayNow++;
            }
