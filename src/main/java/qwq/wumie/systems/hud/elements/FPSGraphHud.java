...
public class FPSGraphHud extends HudElement {
    ...
    public static final HudElementInfo<FPSGraphHud> INFO = new HudElementInfo<>(Hud.GROUP,"fps-graph", "Renders a fps graph of the last 240 ticks.",FPSGraphHud::new);
    ...
    public void tick(HudRenderer renderer) {
        ...
        textX = box.x;
        textY = box.y + box.height - outherOffset.get() * 20;
    }
        ...
        if (renderMode.get() == RenderMode.Default) {
            if (lines.get()) {
                for (int i = (start + displayed) % 240; i != end; i = (i + 1) % 240) {
                    if (i < samples.length - 1 && x <= box.x + width) {
                ...
                for (int i = (start + displayed) % 240; i != end; i = (i + 1) % 240) {
                    if (i < samples.length && x <= box.x + width) {
                        int t = (int) (((double) samples[i] / (1000000000L / 60)) * 30);

                        double tempX = x;
                        double tempY = y + height - 2;
                        double tempWidth = x + factor + 1 >= box.x + width ? x - (box.x + width) - 1 : factor;
