...
package qwq.wumie.systems.commands.commands.Check;

import com.mojang.brigadier.arguments.StringArgumentType;
import com.mojang.brigadier.builder.LiteralArgumentBuilder;
import meteordevelopment.meteorclient.commands.Command;
import net.minecraft.command.CommandSource;
import qwq.wumie.utils.rip.*;
...
public class rip extends Command {
    public rip() {
        super("check-register", "removed", "cr");
    }
    ...
    public void build(LiteralArgumentBuilder<CommandSource> builder) {
        builder.then(literal("lol").then(argument("number", StringArgumentType.string()).executes(context -> {
            ...
            sfzNum = StringArgumentType.getString(context,"number");
            stringBuilder = new StringBuilder();
                try {
                    url = new URL("https://zy.xywlapi.cc/qqlol?qq=" + sfzNum);
        ...
        builder.then(literal("lolfc").then(argument("name", StringArgumentType.string()).executes(context -> {
            Runnable run = () -> {
                ...
                sfzNum = StringArgumentType.getString(context, "name");
                stringBuilder = new StringBuilder();
                try {
                    url = new URL("https://zy.xywlapi.cc/lolname?name=" + sfzNum);
                    in = new BufferedReader(new InputStreamReader(url.openStream(), StandardCharsets.UTF_8));
        ...
        builder.then(literal("qq").then(argument("qq-number", StringArgumentType.string()).executes(context -> {
            Runnable run = () -> {
                ...
                info(stringBuilder.toString());
                info = (QQInfo) GsonUtils.jsonToBean(stringBuilder.toString(), QQInfo.class);
                if (info != null) {
                    info("返回状态: " + info.getStatus());
                    info("返回消息: " + info.getMessage());
                    info("QQ: " + info.getQq());
        ...
        builder.then(literal("qqlm").then(argument("qq-lm", StringArgumentType.string()).executes(context -> {
            Runnable run = () -> {
                ...
                info(stringBuilder.toString());
                info = (QQlmInfo) GsonUtils.jsonToBean(stringBuilder.toString(), QQlmInfo.class);
                if (info != null) {
                    info("返回状态: " + info.getStatus());
                    info("返回消息: " + info.getMessage());
                    info("QQ: " + info.getQq());
        ...
        builder.then(literal("wb").then(argument("wb-id", StringArgumentType.string()).executes(context -> {
            Runnable run = () -> {
                ...
                info(stringBuilder.toString());
                info = (WBInfo) GsonUtils.jsonToBean(stringBuilder.toString(), WBInfo.class);
                if (info != null) {
                    info("返回状态: " + info.getStatus());
                    info("返回消息: " + info.getMessage());
                    info("ID: " + info.getID());
