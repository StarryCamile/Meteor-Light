...
import qwq.wumie.systems.modules.lemon.utils.Task;
...
public class CivBreaker extends Module {
    ...
    private final Setting<Boolean> rightClickEat = sgGeneral.add(new BoolSetting.Builder().name("pause-on-eat").description("Stops breaking the block and starts eating EGapple.").defaultValue(false).build());
    ...
    private final Setting<Boolean> ninja = sgGeneral.add(new BoolSetting.Builder().name("head-placer").description("Places obsidian above target head if its time to place crystal.").defaultValue(false).visible(instant::get).build());
    ...
    public CivBreaker() {
        super(Categories.Combat, "civ-breaker", "");
    }
