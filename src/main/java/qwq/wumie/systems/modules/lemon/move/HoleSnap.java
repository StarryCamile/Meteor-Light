package qwq.wumie.systems.modules.lemon.move;
...
import qwq.wumie.systems.modules.lemon.utils.Hole;
import qwq.wumie.systems.modules.lemon.utils.HoleType;
import qwq.wumie.systems.modules.lemon.utils.HoleUtils;
import qwq.wumie.systems.modules.lemon.utils.LemonUtils;

public class HoleSnap extends Module {
    public HoleSnap() {
        super(Categories.Combat, "hole-snap", "Move to the hole nearby.");
    }
    ...
    private void onRender3D(Render3DEvent event) {
        if (alpha > 0) alpha-=2; else alpha = 0;
        ...
        if (targetHole != null) {
            ...
            int iPath = event.renderer.lines.vec3(linePath.x,linePath.y,linePath.z).color(lineColor.get().a(alpha)).next();
            int iPlayer = event.renderer.lines.vec3(from.x,from.y,from.z).color(lineColor.get().a(alpha)).next();
            int iHole = event.renderer.lines.vec3(to.x,to.y,to.z).color(lineColor.get().a(alpha)).next();
