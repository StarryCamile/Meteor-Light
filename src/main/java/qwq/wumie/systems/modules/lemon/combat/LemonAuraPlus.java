...
import qwq.wumie.systems.modules.lemon.utils.*;
...
import static qwq.wumie.systems.modules.lemon.utils.BlockInfo.*;
import static qwq.wumie.systems.modules.lemon.utils.EntityInfo.*;
import static qwq.wumie.systems.modules.lemon.utils.NewRenderUtils.*;
import static qwq.wumie.systems.modules.lemon.utils.PredictionUtils.*;

public class LemonAuraPlus extends Module {
    ...
    private double getBreakDamage(Entity entity, boolean checkCrystalAge) {
        ...
        double selfDamage = NewCrystalUtils.crystalDamage(mc.player, entity.getPos());
    ...
    private void doPlace() {
        ...
        BlockIterator.register((int) Math.ceil(placeRange.get()), (int) Math.ceil(placeRange.get()), (bp, blockState) -> {
            ...
            double selfDamage = NewCrystalUtils.crystalDamage(mc.player, vec3d);
            ...
            for (Entity entity : mc.world.getEntities()) {
                if (antiFriendPop.get() && entity instanceof PlayerEntity friend && Friends.get().isFriend(friend)) {
                    double friendDamage = NewCrystalUtils.crystalDamage(friend, vec3d);
    ...
    private double getDamageToTargets(Vec3d vec3d, boolean breaking, boolean fast) {
        ...
        if (fast) {
            ...
            if (!(breaking && target.hurtTime > 0))
                damage = NewCrystalUtils.crystalDamage(target, vec3d, predict.get(), predictCollision.get(), predictIncrease.get(), null, ignoreTerrain.get());
        } else {
            for (PlayerEntity target : targets) {
                double dmg = NewCrystalUtils.crystalDamage(target, vec3d, predict.get(), predictCollision.get(), predictIncrease.get(), null, ignoreTerrain.get());
    ...
    public class RenderBlock {
        ...
        public void render(RenderInfo ri, Color sides, Color lines) {
            ...
            NewRenderUtils.render(ri, this.pos, sides, lines, height.get());
            sides.a = preSideA;
            lines.a = preLineA;
        }
