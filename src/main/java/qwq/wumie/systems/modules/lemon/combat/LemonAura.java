package qwq.wumie.systems.modules.lemon.combat;
...
import qwq.wumie.systems.modules.lemon.utils.BDamageUtils;
import qwq.wumie.systems.modules.lemon.utils.BPlayerUtils;
import qwq.wumie.systems.modules.lemon.utils.CrystalUtils;
import qwq.wumie.systems.modules.lemon.utils.TimerUtils;
...
public class LemonAura extends Module {
    ...
    public final Setting<Boolean> doPlace = sgPlace.add(new BoolSetting.Builder()
            .name("place")
            .description("If the CrystalAura should place crystals.")
            .defaultValue(true)
            .build()
    );
    ...
    private final Setting<TargetHelperMode> support = sgPlace.add(new EnumSetting.Builder<TargetHelperMode>()
            .name("target-helper")
            .description("Places a support block in air if no other position have been found.")
            .defaultValue(TargetHelperMode.Disabled)
            .visible(doPlace::get)
            .build()
    );
    ...
    private final Setting<Integer> supportDelay = sgPlace.add(new IntSetting.Builder()
            ...
            .defaultValue(1)
            .min(0)
            .visible(() -> support.get() != TargetHelperMode.Disabled && doPlace.get())
            .build()
    );
    ...
    private final Setting<Integer> renderTime = sgRender.add(new IntSetting.Builder()
            ...
            .range(0,20)
            .sliderRange(0,20)
            .visible(() -> renderMode.get() == RenderMode.Normal || renderMode.get() == RenderMode.Smooth)
            .build()
    );
    ...
    public LemonAura() {
        super(Categories.Combat, "lemon-aura", "Automatically places and attacks crystals.");
    }
    ...
    private void doPlace() {
        ...
        AtomicBoolean isSupport = new AtomicBoolean(support.get() != TargetHelperMode.Disabled);
        ...
        BlockIterator.register((int) Math.ceil(placeRange.get()), (int) Math.ceil(placeRange.get()), (bp, blockState) -> {
            ...
            // Check damage to target and face place
            float damage = getDamageToTargets(vec3d, false, !hasBlock && support.get() == TargetHelperMode.Fast);
    ...
    private void placeCrystal(BlockHitResult result, double damage, BlockPos supportBlock) {
        ...
        // Switch back
        if (autoSwitch.get() == AutoSwitchMode.GhostHand) InvUtils.swap(prevSlot, false);
    }
    ...
    private void onRender(Render3DEvent event) {
        ...
        if (renderMode.get() == RenderMode.Smooth) {
            ...
            event.renderer.box(renderBoxOne, placeSideColor.get(), placeLineColor.get(), shapeMode.get(), 0);
        }
    }
    ...
    private void onRender2D(Render2DEvent event) {
        if (renderMode.get() == RenderMode.None|| placeRenderTimer <= 0 || !renderDamage.get()) return;

        if (renderMode.get() == RenderMode.Smooth) {
            if (renderBoxOne == null) return;
            vec3.set(renderBoxOne.minX + 0.5, renderBoxOne.minY + 0.5, renderBoxOne.minZ + 0.5);
        } else vec3.set(renderPos.getX() + 0.5, renderPos.getY() + 0.5, renderPos.getZ() + 0.5);
