...
public class CityBreaker extends Module {
    ...
    public final Setting<swapMode> autoSwapSetting = sgGeneral.add(new EnumSetting.Builder<swapMode>().name("swap-mode").defaultValue(swapMode.Silent).visible(autoSwap::get).build());
    ...
    public CityBreaker() {
        super(Categories.Combat, "city-breaker", "Automatically breaks target's city block.");
    ...
    public void onDeactivate() {
        if (blockPosTarget != null)
            mc.player.networkHandler.sendPacket(new PlayerActionC2SPacket(PlayerActionC2SPacket.Action.ABORT_DESTROY_BLOCK, blockPosTarget, Direction.UP));
    ...
    private void onTick(final TickEvent.Pre event) {
        ...
        if (TargetUtils.isBadTarget(target, targetRange.get())) {
            ...
            blockPosTarget = null;
            breakTimer = 0;
            info("No target founded! Disabling...");
            toggle();
            return;
        }
        ...
        if (delay() == 0) {
            info("No pick in hotbar! Disabling...");
            toggle();
            return;
        }

        if (blockPosTarget == null) {
            info("No city block found! Disabling...");
            toggle();
            return;
        }

         if (PlayerUtils.distanceTo(blockPosTarget) > mc.interactionManager.getReachDistance()) {
             info("Block to mine too far! Disabling...");
             toggle();
             return;
        }
