...
import qwq.wumie.systems.modules.lemon.utils.BEntityUtils;
import qwq.wumie.systems.modules.lemon.utils.BWorldUtils;
import qwq.wumie.systems.modules.lemon.utils.NewCrystalUtils;
...
public class CevBreaker extends Module {
    ...
    private final Setting<BWorldUtils.PlaceMode> placeMode = sgPlacing.add(new EnumSetting.Builder<BWorldUtils.PlaceMode>()
            .name("place-mode")
            .description("How to switch to your target block.")
            .defaultValue(BWorldUtils.PlaceMode.Client)
            .build()
    );
    ...
    public CevBreaker() {
        super(Categories.Combat, "cev-breaker", "place mine break!");
    }
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        LemonAura BBomber = Modules.get().get(LemonAura.class);
        if(BBomber.bestTarget == null || BBomber.bestTarget != closestTarget || BBomber.BminDamage.get() >= 6) {
            ...
            for(EndCrystalEntity crystal : crystals) {
                if(NewCrystalUtils.crystalDamage(closestTarget, crystal.getPos()) >= 6) {
