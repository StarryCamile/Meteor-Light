...
public class AutoEz extends Module {
    ...
    private final Setting<List<String>> popMessages = sgPops.add(new StringListSetting.Builder()
        ...
        .defaultValue(List.of("ez {player}", "poped {player} with meteor light", "{pops} pops on {player} already"))
        .visible(() -> popMsg.get() && randomMsg.get())
        .build()
    );
    ...
    private final Setting<List<String>> killMessages = sgKills.add(new StringListSetting.Builder()
        ...
        .defaultValue(List.of("ez {player}", "killed {player} with meteor light", "currently at {kills} kill streak", "{playerkills} kills on {player} already", "join meteor light for free: https://discord.gg/A3nYgbKeXR", "{online} people saw {player} die to the power of meteor light", "☠☠", "☠"))
        .visible(() -> killMsg.get() && randomMsg.get())
        .build()
    );
    ...
    public AutoEz() {
        super(Categories.Player, "m-auto-ez", "Send a chat message after killing or poping a player.");
    ...
    private void sendMsg(String string) {
        if (string != null) {
            ...
            if (message.get()) {
                sendChatMessage(smallCaps.get() ? builder.toString() : string);
            } else {
