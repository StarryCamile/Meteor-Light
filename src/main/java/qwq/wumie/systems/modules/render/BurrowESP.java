...
public class BurrowESP extends Module {
    ...
    private void onTick(TickEvent.Post event) {
        ...
        if (TargetUtils.isBadTarget(targetEntity, mc.interactionManager.getReachDistance() + 2)) {
            target = null;
        } else if (renderWebbed.get() && LEntityUtils.isWebbed(targetEntity)) {
            target = targetEntity.getBlockPos();
        } else if (LEntityUtils.isBurrowed(targetEntity, LEntityUtils.BlastResistantType.Any)) {
            target = targetEntity.getBlockPos();
        } else target = null;

        isTargetWebbed = (target != null && LEntityUtils.isWebbed(targetEntity));
        isTargetBurrowed = (target != null && LEntityUtils.isBurrowed(targetEntity, LEntityUtils.BlastResistantType.Any));
    }
