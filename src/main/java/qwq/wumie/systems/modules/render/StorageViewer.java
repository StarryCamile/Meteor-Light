...
public class StorageViewer extends Module {
    ...
    public StorageViewer() {
        super(Categories.Render, "storage-viewer", "Shows you the inventory of storage blocks.");
    }
                ...
                for (BlockEntity entity : entities) {
                    if ((!onlyOnce.get() || onlyOnce.get() && !chests.containsKey(entity.getPos())) && (!ignore.get() || ignore.get() && !positions.contains(entity.getPos()))) {
                        if (storageBlocks.get().contains(entity.getType()) && VectorUtils.distance(mc.player.getPos(), Vec3d.ofCenter(entity.getPos())) <= range.get()) {
                            ...
                            FindItemResultPlus item = VectorUtils.findInHotbar(stack -> stack.getUseAction() == UseAction.NONE);

                            mc.getNetworkHandler().sendPacket(new PlayerInteractBlockC2SPacket((item.getHand() != null && item.found()) ? item.getHand() : Hand.OFF_HAND, new BlockHitResult(Utils.vec3d(position), getDir(position), position, false),0));
    ...
    private void onRender2D(Render2DEvent event) {
        ...
        for (BlockPos pos : new HashSet<>(chests.keySet())) {
            ...
            if (entity == null || !World.isValid(pos) || VectorUtils.distanceXZ(mc.player.getPos(), Vec3d.ofCenter(pos)) >= mc.options.getViewDistance().getValue() * 17) {
                chests.remove(pos);
            } else if (chests.containsKey(pos) && storageBlocks.get().contains(mc.world.getBlockEntity(pos).getType())) {
                Vector3d vec = new Vector3d();
                Vec3d v = Vec3d.ofCenter(pos.up());
                vec = vec.set(v.x,v.y,v.z);
                ...
                if (NametagUtils.to2D(vec, scale)) {
                    ...
                    for (int row = 0; row < 3; row++) {
                        for (int i = 0; i < 9; i++) {
                            ...
                            gameRender.drawItem(event.drawContext,stack, (int) (x + (8 + i * 18) * scale), (int) (y + (7 + row * 18) * scale), (float) scale, true);
                        }
                    }
