...
public class VillagerRoller extends Module {
    ...
    private void fillWidget(GuiTheme theme, WVerticalList list) {
        ...
        save.action = () -> {
            if (saveSearchingToFile(new File(new File(moduleFolder, "villager-roller"), nfname.get()+".nbt"))) {
        ...
        var path = moduleFolder.toPath().resolve("villager-roller");
        ...
        if (fnames.size() != 0) {
            ...
            load.action = () -> {
                if (loadSearchingFromFile(new File(new File(moduleFolder, "villager-roller"), lfname.get()+".nbt"))) {
                    list.clear();
                    fillWidget(theme, list);
                } else {
...
//      Lnet/minecraft/village/TradeOffers$EnchantBookFactory;create(Lnet/minecraft/entity/Entity;Lnet/minecraft/util/math/random/Random;)Lnet/minecraft/village/TradeOffer;
    ...
    public static class EnchantmentSelectScreen extends WindowScreen {
        private final Registry<Enchantment> available = Registries.ENCHANTMENT;
