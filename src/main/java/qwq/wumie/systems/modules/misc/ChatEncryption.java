...
import qwq.wumie.utils.cipher.cipher.EncryptUtils;
...
public class ChatEncryption extends Module {
    ...
    private void onSendPacket(PacketEvent.Send event) {
        if (event.packet instanceof ChatMessageC2SPacket packet && !cancel && (mode.get() == Mode.Encrypt || mode.get() == Mode.Both)) {
            String encrypted = encrypt(packet.chatMessage());

            if (sendOnFail.get() || !packet.chatMessage().equals(encrypted)) {
                cancel = true;
                sendChatMessage(encrypted);
                cancel = false;
        ...
        if (mode.get() == Mode.Decrypt || mode.get() == Mode.Both) {
            ...
            if (string.contains(getDecryptionPrefix()) && string.contains(cipherSuffix.get()) && string.indexOf(getDecryptionPrefix()) < string.indexOf(cipherSuffix.get(), string.indexOf(getDecryptionPrefix()))) {
                try {
                    ...
                    MutableText copy = Text.literal("[Copy]");
                    copy.setStyle(copy.getStyle()
                        ...
                        .withHoverEvent(new HoverEvent(
                            ...
                            Text.literal("Copy the original message.")
                        ))
                    );

                    if (displayMode.get() == DisplayMode.Normal) {
                        MutableText text = Text.literal(string + " ").append(Text.literal("[Decrypted]")
                            ...
                            .append(renderCopy.get() ? Text.literal(" ").append(copy) : Text.literal(""))
                        );

                        event.setMessage(text);
                    } else {
                        MutableText buttons = Text.literal("[Decrypted]")
                            .setStyle(Style.EMPTY.withColor(decryptedColor.get().copy().a(255).getPacked())
                                .withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, Text.literal(string)))
                            )
                            .append(renderCopy.get() ? Text.literal(" ").append(copy) : Text.literal("")
                        );

                        event.setMessage(original.copy().append(" ").append(buttons));
                    }
                } catch (Exception exception) {
                    exception.printStackTrace();
                    if (warnings.get())
