...
import meteordevelopment.meteorclient.events.world.*;
...
import qwq.wumie.utils.world.Ore;
...
public class OreSim extends Module {
    ...
    private List<BlockPos> xrayidk = new ArrayList<>();
    private List<Block> xrayidddd = List.of(
        Blocks.COAL_ORE,
    ...
    public final Setting<Boolean> xrayMode = sgGeneral.add(new BoolSetting.Builder()
        .name("xray-show")
        .defaultValue(false)
            .onChanged((x) -> {
                if (!isActive()) return;
                ...
                if (seed != null) {
                    if (Modules.get().get(Xray.class).isActive()) {
    ...
    public final Setting<Integer> alpha = sgGeneral.add(new IntSetting.Builder()
        .name("alpha")
        .description("The opacity for render color.")
        .defaultValue(25)
        .range(0, 255)
        .sliderMax(255)
        .build()
            ...
            for (Ore ore : oreConfig) {
                if (ore.enabled.get()) {
                    ...
                    for (Vec3d pos : chunkRenderers.get(chunkKey).get(ore)) {
                            Color ax = ore.color;
                            event.renderer.box(pos.x,pos.y,pos.z,pos.x+1,pos.y+1,pos.z+1,new Color(ax.r,ax.g,ax.b,alpha.get()), new Color(ax.r,ax.g,ax.b,alpha.get()), ShapeMode.Both,0);
                    }
                }
            }
        }
    ...
    private boolean exists(BlockPos v) {
            for (BlockPos x : xrayidk) {
    ...
    public void onDeactivate() {
        Seed seed = Seeds.get().getSeed();
        if (seed == null) return;
        if (xrayMode.get()) mc.worldRenderer.reload();
        super.onDeactivate();
    }
    ...
    public void onActivate() {
        if (Seeds.get().getSeed() == null) {
            error("No seed found. To set a seed do .seed <seed> <version>");
            this.toggle();
        }
        reload();
    }
