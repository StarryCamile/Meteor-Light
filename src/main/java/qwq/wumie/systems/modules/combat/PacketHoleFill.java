...
public class PacketHoleFill extends Module {
    ...
    private void onPreTick(TickEvent.Pre event) {
        ...
        if (item.found() && !shouldPause()) {
            ...
            if (!holes.isEmpty()) {
                holes.sort(Comparator.comparingDouble(hole -> VectorUtils.distance(mc.player.getPos(), Vec3d.ofCenter(hole.pos1))));
    ...
    private void place(BlockHitResult result, Hand hand) {
        if (hand != null && result != null && mc.world.getWorldBorder().contains(result.getBlockPos()) && mc.player.getStackInHand(hand).getItem() instanceof BlockItem) {
            ...
            mc.getSoundManager().play(new PositionedSoundInstance(group.getPlaceSound(),
                SoundCategory.BLOCKS,
                (group.getVolume() + 1.0F) / 8.0F,
                group.getPitch() * 0.5F,
                Random.createLocal(),
                result.getBlockPos()));
    ...
    private Direction getPlaceSide(BlockPos pos) {
        for (Direction side : Direction.values()) {
            ...
            if (!state.isReplaceable() && state.getFluidState().isEmpty() && !VectorUtils.isClickable(mc.world.getBlockState(pos.offset(side)).getBlock())) {
                return opposite;
            }
        }
