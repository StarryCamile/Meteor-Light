...
import qwq.wumie.utils.misc.BlockEntityIterator;
import qwq.wumie.utils.misc.FindItemResultPlus;
...
import qwq.wumie.utils.block.BlockPosUtils;
...
public class AutoBed extends Module {
    ...
    private SearchProvider<RecipeResultCollection> container;
    ...
    private void onPreTick(TickEvent.Pre event) {
        ...
        if (mc.world.getDimension().bedWorks() && !ignoreDimension.get()) {
        ...
        if (autoMove.get()) {
            FindItemResultPlus bed = VectorUtils.find(stack -> stack.getItem() instanceof BedItem, 9, 35);
            FindItemResultPlus table = VectorUtils.find(stack -> stack.getItem() == Items.CRAFTING_TABLE, 9, 35);
                            ...
                            for (int z = pZ - horizontal; z <= pZ + horizontal; z++) {
                                for (int y = Math.max(mc.world.getBottomY(), pY - vertical); y <= Math.min(pY + vertical, mc.world.getTopY()); y++) {
                                    ...
                                    if (dX <= horizontal && dY <= vertical && dZ <= horizontal) {
                                        ...
                                        } else if (mc.world.getBlockState(position).isReplaceable()) {
        ...
        for (FakePlayerEntity player : FakePlayerManager.getFakePlayers()) {
    ...
    private void reloadRecipes() {
        if (mc.getSearchProvider(SearchManager.RECIPE_OUTPUT) != null) container = mc.getSearchProvider(SearchManager.RECIPE_OUTPUT);
        ...
        if (container != null) {
            ...
            for (RecipeResultCollection collection : container.findAll("bed")) {
                for (Recipe<?> recipe : collection.getAllRecipes()) {
                    if (recipe.getOutput(DynamicRegistryManager.EMPTY).getItem() instanceof BedItem) {
                        recipes.add(recipe);
                    }
                }
            }
    ...
    private List<Direction> getDirectionsForBlock(BlockPos pos) {
        ...
        for (Direction direction : Direction.values()) {
            if (direction != null && direction != Direction.UP && direction != Direction.DOWN && mc.world.getBlockState(pos.offset(direction)).isReplaceable()) {
    ...
    private Direction getBestSide(BlockPos pos) {
        ...
        for (Direction dir : Direction.values()) {
            if (mc.world.getBlockState(pos.offset(dir)).isReplaceable()) directions.add(dir);
        }
    ...
    private void place(BlockHitResult result, Hand hand, boolean packetPlace, boolean sneak) {
        if (hand != null && result != null && mc.world.getWorldBorder().contains(result.getBlockPos()) && mc.player.getStackInHand(hand).getItem() instanceof BlockItem) {
            if (packetPlace) {
                ...
                mc.getSoundManager().play(new PositionedSoundInstance(group.getPlaceSound(), SoundCategory.BLOCKS, (group.getVolume() + 1.0F) / 8.0F, group.getPitch() * 0.5F,Random.createLocal(),result.getBlockPos()));
                ...
                if (mc.interactionManager.interactBlock(mc.player, hand, result).shouldSwingHand()) swingHand(hand);
    ...
    private boolean isVisible(Vec3d start, BlockPos end, Entity entity, Pair<BlockPos, BlockPos> bed) {
        ...
        BlockHitResult result = BlockView.raycast(raycastContext.getStart(), raycastContext.getEnd(), raycastContext, (context, pos) -> {
            ...
            Vec3d p1 = context.getEnd();
            return BlockHitResult.createMissed(context.getEnd(), Direction.getFacing(vec3d.x, vec3d.y, vec3d.z), new BlockPos(new Vec3i((int) p1.x, (int) p1.y, (int) p1.z)));
        });
    ...
    private BlockHitResult raycast(RaycastContext context, Pair<BlockPos, BlockPos> bed, boolean ignoreTerrain) {
        if (mc == null || mc.world == null || mc.player == null) {
            ...
            return BlockHitResult.createMissed(context.getEnd(), Direction.getFacing(diff.x, diff.y, diff.z), BlockPosUtils.createPos(context.getEnd()));
        }
        ...
        return BlockView.raycast(context.getStart(), context.getEnd(), context, (raycast, pos) -> {
            ...
            return BlockHitResult.createMissed(raycast.getEnd(), Direction.getFacing(diff.x, diff.y, diff.z), BlockPosUtils.createPos(raycast.getEnd()));
        });
    }
    ...
    private double blastProtReduction(Entity player, double damage, Explosion explosion) {
        int level = EnchantmentHelper.getProtectionAmount(player.getArmorItems(), mc.world.getDamageSources().explosion(explosion));
