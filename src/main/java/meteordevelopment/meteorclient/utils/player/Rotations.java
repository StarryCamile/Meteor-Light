...
public class Rotations {
    ...
    private static final Vector3d vec3d1 = new Vector3d();
    private static void set(Vector3d vec,Entity entity, double tickDelta) {
    ...
    public static void rotation(Entity target, double delta, double speed, KAInject.RotationTarget rm) {
        set(vec3d1,target, delta);

        if (rm.equals(KAInject.RotationTarget.Head)) vec3d1.add(0, target.getEyeHeight(target.getPose()), 0);
        if (rm.equals(KAInject.RotationTarget.Body)) vec3d1.add(0, target.getEyeHeight(target.getPose()) / 2, 0);
        ...
        deltaAngle1 = MathHelper.wrapDegrees(angle - mc.player.getPitch());
        toRotate1 = speed * (deltaAngle1 >= 0 ? 1 : -1) * delta;
        if ((toRotate1 >= 0 && toRotate1 > deltaAngle1) || (toRotate1 < 0 && toRotate1 < deltaAngle1)) toRotate1 = deltaAngle1;
        rotate(mc.player.getYaw()+toRotate,mc.player.getPitch() + (float) toRotate1);
    }
