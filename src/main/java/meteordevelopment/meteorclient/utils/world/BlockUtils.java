...
import static qwq.wumie.systems.modules.lemon.utils.BlockInfo.getState;
...
public class BlockUtils {
    ...
    // Placing
    public static boolean collideBlockIntersects(Box bb, Collidable collide) {
        PlayerEntity player = mc.player;
        assert player != null;
        int x = MathHelper.floor(player.getBoundingBox(player.getPose()).minX);
        for (int var3 = MathHelper.floor(player.getBoundingBox(player.getPose()).maxX) + 1; x < var3; ++x) {
            int z = MathHelper.floor(player.getBoundingBox(player.getPose()).minZ);
            for (int var5 = MathHelper.floor(player.getBoundingBox(player.getPose()).maxZ) + 1; z < var5; ++z) {
                BlockPos blockPos = BlockPos.ofFloored((double) x, bb.minY, (double) z);
                Block block = BlockInfo.getBlock(blockPos);
                if (collide.collideBlock(block) && block != null) {
                    Box box = block.getCollisionShape(getState(blockPos),mc.world, blockPos,ShapeContext.absent()).getBoundingBox();
                    if (box != null && !block.getCollisionShape(getState(blockPos),mc.world, blockPos,ShapeContext.absent()).isEmpty()) {
                        if (player.getBoundingBox(player.getPose()).intersects(box)) {
                            return true;
                        }
                    }
                }
