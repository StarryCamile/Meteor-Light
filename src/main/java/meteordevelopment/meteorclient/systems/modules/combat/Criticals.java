...
public class Criticals extends Module {
    ...
    CritInject inject = Injects.getByModuleClass(Criticals.class);

    @EventHandler
    private void onSendPacket(PacketEvent.Send event) {
        if (inject == null) {
            inject = Injects.getByModuleClass(Criticals.class);
        }

        if (!inject.onPacket(mode,event)) {
            return;
        }
        ...
        if (event.packet instanceof IPlayerInteractEntityC2SPacket packet && packet.getType() == PlayerInteractEntityC2SPacket.InteractType.ATTACK) {
            ...
            if (inject.onAttack(mode, (LivingEntity) entity)) {
                switch (mode.get()) {
                    case Packet -> {
                        sendPacket(0.0625);
                        sendPacket(0);
